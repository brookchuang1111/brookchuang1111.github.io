...
*Proof*. Using the Cramer-Chernoff method, let $\lambda > 0$ to be some constant later. Then,
...
We can choose $\lambda = \epsilon// \sigma^2$ to complete the inequality and proof.
...
Ok let's get back to the algorithm. ETC is characterized by the number of times it explores each arm, denoted $m$. Because there are $k$ actions, the total number of times ETC explores is $mk$ rounds until committing. The *average reward* is denoted as $\hat{\mu}_i(t)$ such that, 
...
With this we can bound our regret given our suboptimality gap $\Delta_i = \mu* - \mu_i$ and mean reward $\mu_i$[^2].
...
Substitute $2.3$ into $2.1$ $R_n$ and $2.2$ $\mathbb{E}[T_i(n)]$ for the result. 
...
where $C > 0$ is a universal constant and when $\Delta \leq 1$ it is often assumed. We also have another bound, 
...
This is the simplest and what my mentor calls the most "stupid" bandit algorithm but it's still good to review for my own sake. In some ways, this parallels any betting game: when do we access the trade-off to commit to a strategy versus explore different strategies? What's our EV (or in this case our constructed bound) for finding the optimal reward and sticking to such reward? Interesting thoughts I might explore later.
