...
*A backtest will only be realistic if we use hisotrical data extracted from the same venue(s) as the one(s) we expect to trade on.*
- trade prices and sizes are not generally available, but we can use bid-ask quotes for backtesting forex strategies since the spreads for the same currency pair can vary signifigantly between venues.
